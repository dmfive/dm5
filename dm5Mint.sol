...
contract Minter {
   ...
   address public admin;
    ...
    constructor(address _token,address _dev,address _admin) public {
        require(address(_token) != address(0),"_mainToken is zero value!");
        require(address(_dev) != address(0),"_dev is zero value!");
        require(address(_admin) != address(0),"_admin is zero value!");
        dev = _dev;
        admin = _admin;
        dm5 = ContractConn(_token);
    }    
    ...
    function claim(uint256 orderId, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public  {
        ...
        dm5.transfer(address(msg.sender), amount.mul(99).div(100));
